...
def save_retry(fn, max_attempts=15):
...
def main():
    ...
    save_retry(lambda: res_df.to_csv(os.path.join(out_dir, "models_comparison.csv"), index=False))
    ...
    present_names = [class_names[c] for c in classes]

    M.plot_confusion_matrix(y_te, p_te, present_names,
    ...
    save_retry(lambda: joblib.dump(bundle, os.path.join(out_dir, "model_bundle.joblib")))
