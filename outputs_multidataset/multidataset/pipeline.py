...
def process_one(cfg, model, row, overlay_dir=None, return_display=False):
    ...
    if bgr.ndim != 3 or bgr.size == 0 or float(bgr.std()) < 1.0:
        return None, f"degenerate image {path} (std={float(bgr.std()):.2f})"
    ...
    out.update(feats)
    if return_display:
        out["_av_display"] = (np.clip(pred, 0, 1) * 255).astype(np.uint8)
        labels = les.get("_labels")
        if labels is not None:
            comp = np.zeros((*labels.shape, 3), dtype=np.uint8)
            for lbl, color in ((1, (255, 0, 255)), (2, (0, 255, 255)),
                               (3, (0, 0, 255)), (4, (0, 255, 0))):
                comp[labels == lbl] = color
            out["_lesion_display"] = comp
    return out, None
